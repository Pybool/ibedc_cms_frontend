...
import { ecmiCustomers } from '../pages/customersmodule/prepaidcustomers/state/customer.selector';
...
export class CustomerService {
  ...
  nextEnergyPage(page,link){
    console.log("Page n link ==> ",page, link)
    return this.http.get<any>(`${environment.api}/customers/meter-readings${link}&cuid=${this.user_id}&offset=${page}`)
    .pipe(
      timeout(environment.requestTimeOut), // Timeout after 20 seconds
      catchError((error) => {
        // Handle errors here
  ...
  fetchEnergyReading(payload){
    return this.http.get<any>(`${environment.api}/customers/meter-readings?msno=${payload.msno || ''}&year=${payload.year}&month=${payload.month}&event_type=${payload.eventType}&cuid=${this.user_id}&offset=${1}`)
    .pipe(
      timeout(environment.requestTimeOut), // Timeout after 20 seconds
      catchError((error) => {
        // Handle errors here
